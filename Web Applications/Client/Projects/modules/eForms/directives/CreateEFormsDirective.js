		...
		link: function(scope, ele, attr) {
					...
					switch(idDirective) {
					   case 1:
					    ...
					    case 13:
					        angular.element(document.getElementById(idSection))
							.append($compile("<div class='col-md-12'>&nbsp;</div><div class='col-md-12' id='"+idSection+"."+questionName+
								"' ><question13 on-option='"+modelValue+"' on-editor='true'></div>")(scope));
					        break;
					    default:
					        alert("Error");
					}
			...
			*/
			...
			scope.update = function() {
				console.log(scope.ObjectMap);
				// eFormService.upadteTemplate(scope.ObjectMap)
				// .then(function(result){
				// 	console.log(result);
				// },function(err){
				// 	console.log(err);
			...
			scope.init = function() {
				if(scope.TemplateUID != null && scope.TemplateUID != "") {
					...
					.then(function(result) {
						...
						scope.ObjectMap = {
					    	DocName     : result.data.Name, //null,//Ten cua document template duoc tao ra, ( bay h gan mac dinh cho no )
